...
class WarmUpAndDecayScheduler(LearningRateSchedule):
    def __init__(self, initial_learning_rate, warmup_steps, decay_steps, decay_rate):
        ...
        self.decay_scheduler = ExponentialDecay(initial_learning_rate, decay_steps, decay_rate)
        if self.decay_steps <= 0:
            raise ValueError(f"Argument 'decay_steps' must be > 0. Received: decay_steps={decay_steps}")
